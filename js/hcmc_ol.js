...
hol.captions['en'].strReadMore           = 'Ver';
...
// Idioma Español...
hol.captions['es'] = {};
hol.captions['es'].strCloseX             = '×';
hol.captions['es'].strFile               = 'Fichero';
hol.captions['es'].strLoadFile           = 'Cargar fichero';
hol.captions['es'].strPasteGeoJSON       = 'Pegar GeoJSON...'
hol.captions['es'].strPasteHere          = 'Paste a single GeoJSON Feature here';
hol.captions['es'].strSaveGeoJSON        = 'Guardar GeoJSON...';
hol.captions['es'].strSetup              = 'Configurar';
hol.captions['es'].strMapArea            = 'Área en el Mapa';
hol.captions['es'].strDraw               = 'Dibujar';
hol.captions['es'].strLoad               = 'Cargar';
hol.captions['es'].strLoading            = 'Cargando...';
hol.captions['es'].strInfo               = 'ℹ';
hol.captions['es'].strTrack              = '⌖';
hol.captions['es'].strMenuToggle         = '≡';
hol.captions['es'].strOk                 = '✔';
hol.captions['es'].strEdit               = '🖉';
hol.captions['es'].strLocationsByCat     = 'Ciclones Tropicales';
hol.captions['es'].strSearch             = '🔍';
hol.captions['es'].strReadMore           = 'Ver';
hol.captions['es'].strSearchForLocs      = 'Buscar por nombre';
hol.captions['es'].strUnnamedFeat        = 'unnamed feature';
hol.captions['es'].strNetworkError       = 'There was a network error.';
hol.captions['es'].strToggleTracking     = 'Toggle tracking of my location on the map.';
hol.captions['es'].strShowHideAllFeats   = 'Show/hide all features';
hol.captions['es'].strGeoLocNotSupported = 'Sorry, your browser does not support geolocation tracking.';
hol.captions['es'].strGeoLocRequiresTLS  = 'Geolocation tracking requires a secure connection (https).';
hol.captions['es'].strNewTaxonomy        = 'Añadir Taxonomía';
hol.captions['es'].strNewCategory        = 'Añadir Categoría';
hol.captions['es'].strGetTaxonomyName    = 'Type a name for your new taxonomy:'
hol.captions['es'].strGetCategoryName    = 'Type a name for your new category:';
hol.captions['es'].strGetCategoryDesc    = 'Type a brief description for your new category:';
hol.captions['es'].strGetFeatureName     = 'Type a name for your new feature:';
hol.captions['es'].strStopDrawing        = 'Parar de dibujar';
hol.captions['es'].strClear              = 'Limpiar';
hol.captions['es'].strDrawnFeatures      = 'Drawn features';
hol.captions['es'].strDrawnFeaturesDesc  = 'Features drawn during the current session';
hol.captions['es'].strEditThisFeature    = 'Edit a copy of this feature by clicking on the edit button above.';
...
//Setting for testing new features.
    ...
    this.captionLang = document.getElementsByTagName('html')[0].getAttribute('lang') || 'es'; //Document language.
    ...
    this.timelineButton = null;                       //Pointer to Information button after we have created it.
...
//Next we create a div for displaying external retrieved documents.
    ...
    closeBtn.addEventListener('click', function(e){e.target.parentNode.style.left = '-41rem'; this.docDisplayFrame.setAttribute('src', '');}.bind(this), false);
...
//Add an event listener to fix hol: links whenever a document is loaded.
    ...
    this.timelineDisplayDiv = document.createElement('div');
	this.timelineDisplayDiv.setAttribute('id', 'timelineDisplay');
	timelineClose = document.createElement('span');
    timelineClose.setAttribute('class', 'closeBtn');
    timelineClose.appendChild(document.createTextNode(this.captions.strCloseX));
	
	timelineClose.addEventListener('click', function(e){
		e.target.parentNode.style.left = '-91rem';
		this.timelineButton.disabled = false; 
		this.timelineFrame.setAttribute('src', '');
		 
		document.getElementById('holRightBox').classList.toggle('hidden'); 
		this.mobileMenuToggleButton.classList.toggle('menuHidden');
	}.bind(this), false);

	this.timelineDisplayDiv.appendChild(timelineClose);
    this.timelineFrame = document.createElement('iframe');
    this.timelineFrame.setAttribute('id', 'timelineFrame');
    this.timelineDisplayDiv.appendChild(this.timelineFrame);
    this.docBody.appendChild(this.timelineDisplayDiv);
    
//Add an event listener to fix hol: links whenever a document is loaded.
    this.timelineFrame.addEventListener('load', function(){try{this.rewriteHolLinks(this.timelineFrame.contentDocument.getElementsByTagName('body')[0]);}catch(e){}}.bind(this), false);
...
hol.VectorLayer.prototype.buildToolbar = function(){
  ...
  try{
    ...
    this.timelineButton.appendChild(document.createTextNode('T'));
    this.timelineButton.addEventListener('click', this.showTimeline.bind(this,'html/cronology/timeline.html'));
    this.toolbar.appendChild(this.timelineButton);
...
//Sanity check:
    if (this.currTaxonomy < 0){
      throw new Error('Sin taxonomía para mostrar; current taxonomy set to ' + this.currTaxonomy + '.');
    }
...
hol.VectorLayer.prototype.setSelectedFeature = function(featNum, jumpInNav){
  var currFeat, props, div, showDoc, readMore, targetCat, catLi, featLi, i, imax;
...
//Show the edit button if we're allowing feature editing.
    ...
    if ((props.links.length > 0)&&(this.infoDiv.querySelectorAll('span[class=\'holShowDoc\']').length < 1)){
      ...
      for (i=0, imax=props.links.length; i< imax; i++){
        showDoc = document.createElement('a');
        ...
        div.appendChild(showDoc);
      }
      
      this.infoDiv.querySelector("div[id='infoContent']").appendChild(div);
    }
    this.infoDiv.style.display = 'block';
...
//Hide the document display box.
  ...
  this.docDisplayDiv.style.left = '-41rem';
...
hol.VectorLayer.prototype.showTimeline = function(docPath){
  try{
    document.getElementById('holRightBox').classList.toggle('hidden'); 
	this.mobileMenuToggleButton.classList.toggle('menuHidden');
	
	this.timelineButton.disabled = true; 
	
	this.timelineFrame.setAttribute('src', this.linkPrefix + docPath);
    this.timelineDisplayDiv.style.left = '0';
